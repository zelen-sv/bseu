<template>
  <div class="documents-list content-section" id="admission-documents">
    <div class="documents-list__title content-section__legend">
      {{ $t("pages.second-higher.admission-documents.legend") }}
    </div>
...
<script>
  import visaDocuments from '~/components/second-higher/visaDocuments.vue'
  import noVisaDocuments from '~/components/second-higher/noVisaDocuments.vue'
  ...
  export default {
    ...
    computed: {
      visaMode () {
        return this.$store.getters.secondHigherPageViseMode
      }
    },
    methods: {
      setVisaMode (state) {
        if (state) {
          this.$store.dispatch('setVisaMode', 'secondHigher')
        } else {
          this.$store.dispatch('setVisaFreeMode', 'secondHigher')
        }
      },
    }
  }
